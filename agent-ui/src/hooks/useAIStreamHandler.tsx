...
const useAIChatStreamHandler = () => {
  ...
  const handleStreamResponse = useCallback(
    async (input: string | FormData) => {
      ...
      try {
        ...
        await streamResponse({
          ...
          onChunk: (chunk: RunResponse) => {
            if (
              ...
              if (chunk.run_id) {
                useStore.getState().setCurrentRunId(chunk.run_id)
              }
              if (
        ...
        focusChatInput()
        setIsStreaming(false)
        useStore.getState().setCurrentRunId(null)
  ...
  const cancelRun = useCallback(async () => {
    const { abortController, currentRunId, mode } = useStore.getState()
    const [agentIdValue] = [agentId]
    const [teamIdValue] = [teamId]

    // 1. Abort the client-side stream
    if (abortController) {
      abortController.abort()
    }

    // 2. Notify backend to cancel server-side processing
    if (currentRunId) {
      try {
        if (mode === 'agent' && agentIdValue) {
          await cancelAgentRunAPI(
            constructEndpointUrl(selectedEndpoint),
            agentIdValue,
            currentRunId,
            authToken
          )
        } else if (mode === 'team' && teamIdValue) {
          await cancelTeamRunAPI(
            constructEndpointUrl(selectedEndpoint),
            teamIdValue,
            currentRunId,
            authToken
        ...
        // Backend cancel is best-effort — stream is already aborted client-side
      }
    }
    ...
    // 4. Mark the last agent message as cancelled (add a cancelled indicator)
    setMessages((prevMessages) => {
      const newMessages = [...prevMessages]
      const lastMessage = newMessages[newMessages.length - 1]
      if (lastMessage && lastMessage.role === 'agent') {
        if (!lastMessage.content || lastMessage.content.trim() === '') {
          lastMessage.content = '(Run cancelled)'
        }
        lastMessage.streamingError = false  // Not an error — intentional cancel
      }
      return newMessages
    })

    focusChatInput()
  }, [agentId, teamId, selectedEndpoint, authToken, setIsStreaming, setStreamingErrorMessage, setMessages, focusChatInput])
